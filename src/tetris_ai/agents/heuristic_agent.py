...
class TetrisGoal:
    ...
    def evaluate_board(
        ...
        game_over = self._is_game_over_observation(observation, termination_reason)
    ...
    def evaluate_state(self, state: TetrisSearchState) -> float:
        ...
        if metrics is None:
            raise ValueError("HeuristicSearchAgent requires observation_mode='featured'.")
        game_over = self._is_game_over_state(state)
        return self._evaluate_metrics(metrics.aggregate_height, metrics.holes, game_over)
    ...
    @staticmethod
    def _has_game_over_reason(termination_reason: str | None) -> bool:
        return bool(termination_reason and "game_over" in termination_reason.split("+"))

    def _is_game_over_observation(
        self,
        observation: Observation,
        termination_reason: str | None,
    ) -> bool:
        # A finite experimental horizon is terminal for the MDP, but it is not
        # a Tetris loss.  Only explicit or unambiguous top-out receives the
        # game-over penalty.
        return self._has_game_over_reason(termination_reason) or (
            termination_reason is None
            and observation.terminated
            and observation.remaining_pieces > 0
        )

    def _is_game_over_state(self, state: TetrisSearchState) -> bool:
        return self._has_game_over_reason(state.termination_reason) or (
            state.termination_reason is None
            and state.terminated
            and state.remaining_pieces > 0
        )
...
class HeuristicSearchAgent(Agent):
    ...
    def configuration(self) -> dict[str, object]:
        return {
            "policy": "state_goal_best_first_search",
